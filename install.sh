...
PREV_INSTALL=0
REMOVE_EXISTING=0
UNIT_FILE="/etc/systemd/system/pktpcap.service"
...
if [ "$PREV_INSTALL" -eq 1 ]; then
    ...
    echo "Found an existing pktPCAP installation at $INSTALL_DIR (version $PREV_VERSION)."
    ...
    elif [ -n "${PKTPCAP_REMOVE_EXISTING:-}" ]; then
        REMOVE_EXISTING="$PKTPCAP_REMOVE_EXISTING"
    elif [ -t 0 ]; then
