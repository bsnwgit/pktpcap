"""
app/api/suite.py — pktHub integration endpoints (inbound: pktHub or a
sibling app calling into pktPCAP).

Token flow:
  1. pktPCAP generates a random suite_token on first call to GET /api/suite/token
  2. Admin copies the token from Settings -> Security -> Suite Integration
...
log = logging.getLogger("pktpcap.api.suite")
