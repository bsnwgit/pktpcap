...
----------------------------------
Base HTTP client for pktPCAP acting as a suite-token CLIENT of a sibling
pkt* app. This is the mirror image of app/api/suite.py (which is the
INBOUND side — pktHub calling into pktPCAP).
...
suite_token:
  X-Suite-Token — the token copied from that app's Settings -> Security -> Suite Integration page
...
log = logging.getLogger("pktpcap.integrations")
