...
// Integration, a dual-provider (Anthropic/OpenAI) AI assistant, and SSL/TLS.
...
export default function Settings() {
  ...
  const aiAssistantSave = useSave([
    ...
    'ai_local_providers',
    'ai_provider_anthropic_enabled', 'anthropic_key', 'anthropic_model',
    'ai_provider_openai_enabled', 'openai_key', 'openai_model',
  ], settings, load)
            ...
            {securityTab === 'ai' && (
              <Section title="AI Assistant" onSave={aiAssistantSave.save} saving={aiAssistantSave.saving} saved={aiAssistantSave.saved} error={aiAssistantSave.error}
                help={{
                  ...
                  content: <>
                    <p>Providers are tried top to bottom — <span className="text-gray-300 font-medium">local / self-hosted providers first</span>, since they keep your capture data on hardware you control, then cloud providers as a fallback. Turn on as many as you like; the first enabled provider with valid config answers each question in the chat panel.</p>
                    <p className="mt-2"><span className="text-gray-300 font-medium">Ollama</span> and other local endpoints run entirely on your own hardware — nothing is sent off-network. <span className="text-gray-300 font-medium">Anthropic</span> and <span className="text-gray-300 font-medium">OpenAI</span> are paid, cloud-hosted APIs and each need their own key.</p>
                  </>,
                }}
              >
                ...
                >
                  <MiniField label="API key" hint="Get a key at console.anthropic.com">
                    <TextInput value={str('anthropic_key')} onChange={v => set('anthropic_key', v)} placeholder="sk-ant-…" secret mono />
                  </MiniField>
                  <MiniField label="Model">
                    <SelectInput
                      value={str('anthropic_model', 'claude-opus-4-8')}
                ...
                >
                  <MiniField label="API key" hint="Get a key at platform.openai.com">
                    <TextInput value={str('openai_key')} onChange={v => set('openai_key', v)} placeholder="sk-…" secret mono />
                  </MiniField>
                  <MiniField label="Model">
