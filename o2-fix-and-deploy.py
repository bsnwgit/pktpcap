"""
Fix pktanalyzer on O2: upload fixed server.py, restart service, verify.
...
LOCAL_SERVER   = r"C:\Users\user\My Drive\Documents\Claude\Projects\Packet Analyzer\service\server.py"
REMOTE_SERVER  = "/mnt/software/pktanalyzer/server.py"
LOCAL_SETTINGS = r"C:\Users\user\My Drive\Documents\Claude\Projects\Packet Analyzer\service\templates\settings.html"
REMOTE_SETTINGS = "/mnt/software/pktanalyzer/templates/settings.html"
...
safe_print("\n=== Last 20 log lines ===")
out, _ = ssh_run(client, "sudo journalctl -u pktanalyzer -n 20 --no-pager --output=cat")
safe_print(out)
...
# Restart service
safe_print("\n=== Restarting pktanalyzer service ===")
out, err = ssh_run(client, "sudo systemctl restart pktanalyzer")
...
out, _ = ssh_run(client, "sudo journalctl -u pktanalyzer -n 10 --no-pager --output=cat")
safe_print(out)
