#!/bin/bash
# pktPCAP uninstall script — Ubuntu Server 22.04/24.04 LTS
...
# credential encryption key), pktpcap.db and its -wal/-shm, logs, backups and
...
# WorkingDirectory, then $PKTPCAP_INSTALL_DIR, then /opt/pktpcap.

set -euo pipefail

APP="pktpcap"
DISPLAY="pktPCAP"
UNIT_PATH="/etc/systemd/system/pktpcap.service"
...
# migration — plain *.db does not match "pktpcap.db.pre-upgrade".
...
if [ "$PURGE" -eq 1 ]; then
    for entry in logs backups ssl data captures; do
    ...
    for entry in config.yaml pktpcap.db logs backups ssl captures; do
        if [ -e "$INSTALL_DIR/$entry" ]; then
            echo "    $entry"
        fi
    done
