...
def get_settings():
    ...
    # Reflect actual SSL file state so the UI matches reality
    ssl_crt = BASE / "ssl" / "server.crt"
    ssl_key = BASE / "ssl" / "server.key"
    if ssl_crt.is_file() and ssl_key.is_file():
        masked["ssl_enabled"] = True
        masked["ssl_cert"] = str(ssl_crt)
        masked["ssl_key"]  = str(ssl_key)
...
def _save_ssl_file(file_obj, dest_path):
    """Save an uploaded file object to dest_path."""
...
def _pfx_to_pem(pfx_path, passphrase=""):
    """Extract PEM cert and key from a PFX/P12 file using the openssl CLI."""
    import subprocess, tempfile, os as _os
    pfx_path = str(pfx_path)
    pass_arg = "pass:" + (passphrase or "")

    def _run_openssl(args):
        # try with -legacy first (OpenSSL 3+), fall back without it
        for extra in (["-legacy"], []):
            r = subprocess.run(
                ["openssl", "pkcs12"] + extra + args + ["-passin", pass_arg],
                capture_output=True, text=True
            )
            if r.returncode == 0:
                return r
        return r  # return last failed result for error message

    with tempfile.TemporaryDirectory() as tmpdir:
        cert_tmp = _os.path.join(tmpdir, "cert.pem")
        key_tmp  = _os.path.join(tmpdir, "key.pem")

        rc = _run_openssl(["-in", pfx_path, "-clcerts", "-nokeys", "-out", cert_tmp])
        if rc.returncode != 0:
            raise RuntimeError("PFX extraction failed: " + (rc.stderr or "openssl error"))

        rk = _run_openssl(["-in", pfx_path, "-nocerts", "-nodes", "-out", key_tmp])
        if rk.returncode != 0:
            raise RuntimeError("PFX key extraction failed: " + (rk.stderr or "openssl error"))

        return open(cert_tmp, "rb").read(), open(key_tmp, "rb").read()
...
def upload_ssl_file():
    """
    Accepts two modes:
      role=pfx  — single file field 'file', optional passphrase field
      role=pem  — two file fields: 'cert' and 'key'
    After saving, updates ssl_cert / ssl_key in the database so the next
    restart picks up the correct paths.
    """
    ...
    if role == "pfx":
        ...
        if ext not in {".pfx", ".p12"}:
            return jsonify({"ok": False, "error": "Expected .pfx or .p12 file"}), 400
        passphrase = request.form.get("passphrase", "")
        pfx_path = certs_dir / "ssl.pfx"
        f.save(str(pfx_path))
        log.info("PFX uploaded: %s", pfx_path)
        # Convert to PEM so Flask can load them
        try:
            cert_pem, key_pem = _pfx_to_pem(pfx_path, passphrase)
        ...
        # Persist passphrase and paths
        db.set_many_settings({
            "pfx_path":       str(pfx_path),
        ...
        log.info("SSL PEM files written from PFX: cert=%s key=%s", cert_path, key_path)
        return jsonify({"ok": True, "cert": str(cert_path), "key": str(key_path)})

    elif role == "pem":
        cert_f = request.files.get("cert")
        key_f  = request.files.get("key")
        if not cert_f or not cert_f.filename:
            return jsonify({"ok": False, "error": "No cert file provided"}), 400
        if not key_f or not key_f.filename:
            return jsonify({"ok": False, "error": "No key file provided"}), 400
        ssl_dir = BASE / "ssl"
        ssl_dir.mkdir(exist_ok=True)
        cert_path = ssl_dir / "server.crt"
        key_path  = ssl_dir / "server.key"
        cert_f.save(str(cert_path))
        key_f.save(str(key_path))
        db.set_many_settings({
            "ssl_cert":    str(cert_path),
            "ssl_key":     str(key_path),
            "ssl_enabled": True,
        })
        log.info("PEM cert/key installed: cert=%s key=%s", cert_path, key_path)
        return jsonify({"ok": True, "cert": str(cert_path), "key": str(key_path)})

    else:
        return jsonify({"ok": False, "error": "role must be pfx or pem"}), 400


@app.route("/api/ssl/status", methods=["GET"])
def ssl_status():
    """Return current SSL certificate status."""
    # Check the canonical ssl/ directory first (matches pktflow pattern)
    ssl_dir   = BASE / "ssl"
    cert_path = str(ssl_dir / "server.crt")
    key_path  = str(ssl_dir / "server.key")
    if not (os.path.isfile(cert_path) and os.path.isfile(key_path)):
        cfg = load_config()
        cert_path = cfg.get("ssl_cert", "")
    enabled = os.path.isfile(cert_path) and os.path.isfile(str(ssl_dir / "server.key")) if cert_path else False
    if cert_path and os.path.isfile(cert_path):
        info = {"installed": True, "enabled": enabled, "path": cert_path}
        try:
            import ssl as _ssl
            ctx = _ssl.create_default_context()
            ctx.check_hostname = False
            ctx.verify_mode = _ssl.CERT_NONE
            raw = _ssl.PEM_cert_to_DER_cert(open(cert_path).read())
            import datetime
            cert = _ssl.DER_cert_to_PEM_cert(raw)
            # Parse expiry via cryptography if available
            try:
                from cryptography import x509
                from cryptography.hazmat.backends import default_backend
                c = x509.load_pem_x509_certificate(cert.encode(), default_backend())
                info["subject"] = c.subject.rfc4514_string()
                info["expires"] = c.not_valid_after_utc.strftime("%Y-%m-%d")
            except Exception:
                pass
        except Exception:
            pass
        return jsonify(info)
    return jsonify({"installed": False, "enabled": enabled})


@app.route("/api/ssl/remove", methods=["DELETE"])
def ssl_remove():
    """Remove installed SSL certificate and disable SSL."""
    get_db().set_many_settings({"ssl_cert": "", "ssl_key": "", "ssl_enabled": False})
    certs_dir = BASE / "certs"
    for name in ("ssl.crt", "ssl.key", "ssl.pfx"):
        p = certs_dir / name
        if p.exists():
            p.unlink()
...
@app.route("/api/restart", methods=["POST"])
def api_restart():
    """Restart the service via systemctl (runs async so response can be sent first)."""
    import subprocess
    def _do_restart():
        time.sleep(1)
        subprocess.Popen(["systemctl", "restart", "pktpcap"])
...
if __name__ == "__main__":
    ...
    # SSL: check ssl/server.crt + ssl/server.key first (matches pktflow/pktsnmp pattern),
    # then fall back to db-configured paths.
    SSL_DIR  = BASE / "ssl"
    ssl_cert = str(SSL_DIR / "server.crt")
    ssl_key_ = str(SSL_DIR / "server.key")

    if not (os.path.isfile(ssl_cert) and os.path.isfile(ssl_key_)):
        # Fall back to db-configured paths
    ...
    if ssl_cert and ssl_key_ and os.path.isfile(ssl_cert) and os.path.isfile(ssl_key_):
        ssl_context = (ssl_cert, ssl_key_)
        scheme = "https"
        log.info("SSL enabled: cert=%s", ssl_cert)
    else:
        log.info("No SSL cert found — starting HTTP")
